...
extension BasicProperties: POPAnimationDelegate {
    ...
    private func animator(_ name:String!)->POPPropertyAnimation{
...
//            debugPrint("delay:\(self.delayTime)", appendNewline: true)
            if self.delayTime > 0 {
                DispatchQueue.main.after(when: DispatchTime.now() + Double(Int64(self.delayTime
...
public extension BasicProperties {
    ...
    public func done(_ done:NextAnimtionBlock){
        self.doneBlock = done
    }
}
